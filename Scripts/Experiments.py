...
def combine_results(dataset, experiment, rounds, keys):
    ...
    for idx in range(len(keys)):
        ...
        federated = federated.rename(index=str,
                                     columns={"Federated Train Loss": "Federated Train Loss {} {}".format(experiment,
                                                                                                          keys[-idx-1]),
                                              "Federated Train Accuracy": "Federated Train Accuracy {} {}".format(
                                                  experiment,
                                                  keys[-idx-1]),
                                              "Federated Test Loss": "Federated Test Loss {} {}".format(experiment,
                                                                                                        keys[-idx-1]),
                                              "Federated Test Accuracy": "Federated Test Accuracy {} {}".format(experiment,
                                                                                                            keys[-idx-1])})
...
def experiment_1_number_of_clients(dataset, rounds, clients):
    ...
    # Perform Experiments
    experiment_centralized(dataset, train_data[:500], train_labels[:500], test_data, test_labels,
                           rounds)

    for client_num in clients:
        experiment_federated(client_num, dataset, train_data[:500], train_labels[:500], test_data,
                             test_labels,
                             rounds)
