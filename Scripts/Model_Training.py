    ...
    for comm_round in range(global_epochs):
        ...
        if local_personalization:
            ...
            for test_person in np.unique(test_people):
                client_test_data, client_test_labels = split_test_data.get(test_person), \
                                                       split_test_labels.get(test_person)
                weights_accountant.apply_client_weights(model, test_person)
